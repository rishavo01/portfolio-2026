...
This is the `frontend/` half of the project — see the [root README](../README.md) for how it fits together with `backend/` (the contact-form email API).
...
The contact form on the Contact section calls the API in `../backend` (`http://localhost:5000` by default, overridable via `VITE_API_URL` in `.env`) — start that separately for the form to actually send email.
